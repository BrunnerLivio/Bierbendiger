...
module.exports = function (grunt) {
    ...
    grunt.initConfig({
        ...
        browserSync: {
            dev: {
                ...
                options: {
                    proxy: '127.0.0.1:8010/api', //our PHP server
                    port: 8081, // our new port
                    open: true,
                    watchTask: true
                }
            }
        ...
        replace: {
            apipath: {
                ...
                replacements: [{
                    from: 'http://localhost:8081/api/',                   // string replacement
                    to: '/api/'
                }]
            },
            servsidemode: {
                src: ['<%= yeoman.dist %>/api/*.json'],             // source files array (supports minimatch)
            ...
            releasecomments: {
                ...
                dest: '<%= yeoman.dist %>/scripts/',             // destination directory or file
                replacements: [{
                    from: '/*<RELEASE>',                   // string replacement
                    to: ''
                },
                    {
                        from: '</RELEASE>*/',                   // string replacement
    ...
    grunt.registerTask('client', 'Compile then start a connect web server', function (target) {
        if (target === 'dist') {
